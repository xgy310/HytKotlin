...
public class DrawableHelper {
     ...
     */
    @TargetApi(16)
    public static GradientDrawable createCircleGradientDrawable(@ColorInt int startColor, @ColorInt int endColor, int radius,
                                                                @FloatRange(from = 0f, to = 1f) float centerX, @FloatRange(from = 0f, to = 1f) float centerY) {
    ...
    Drawable getVectorDrawable(Context context, @DrawableRes int resVector) {
        try {
            ...
            //QMUILog.d(TAG,
            //          "Error in getVectorDrawable. resVector=" + resVector + ", resName=" + context.getResources().getResourceName(resVector) + e.getMessage());
            return null;
        }
    }
